    ...
    return (
      <Card>
        <CardContent className="p-2">
          <p>{formatDateTime(new Date(label)).dateOnly}</p>
...
const STROKE_COLORS: Record<string, Record<'light' | 'dark', string>> = {
...
export default function SalesAreaChart({ data }: SalesAreaChartProps) {
  ...
  const strokeColor = STROKE_COLORS[color.name]?.[theme || 'light'] || '#000'
